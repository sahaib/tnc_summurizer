...
class TnCSummarizerApp:
    ...
    def summarize(self):
        ...
        try:
            ...
            summary = self.get_summary_from_claude(tnc_text)
    ...
    def get_summary_from_claude(self, text):
        try:
            anthropic = Anthropic(api_key='your-anthropic-api-key-here')  # Replace with your actual Anthropic API key
            
            print(f"Sending request to Claude (first 500 characters): {text[:500]}")
            
            response = anthropic.completions.create(
                model="claude-2",
                max_tokens_to_sample=1000,
                prompt=f"{HUMAN_PROMPT}You are an expert in summarizing Terms and Conditions and Privacy Policies. Your task is to provide clear, concise summaries focusing on key points that users should be aware of. Use relevant emojis at the start of each point.

Please summarize the following Terms and Conditions or Privacy Policy. Focus on these key aspects:
...
{text[:4000]}{AI_PROMPT}",
            ...
            print(f"Received summary from Claude (first 500 characters): {summary[:500]}")
            return summary
        except Exception as e:
            print(f"Error in getting summary from Claude: {str(e)}")
            raise Exception(f"Error in getting summary from Claude: {str(e)}")
